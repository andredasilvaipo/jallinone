 ...
 * <p>Description: Help class used to update document totals, according to the specified activities.</p>
 ...
 * @version 1.0
 */
public class UpdateSaleDocTotalActivityBean {
  ...
  public static final Response updateTotals(SaleDocPK docPK,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
    ...
    try {
      ...
      Logger.error(userSessionPars.getUsername(),"org.jallinone.sales.documents.activities.server.UpdateSaleDocTotalActivityBean","updateTotals","Error while updating document totals:\n"+ex.getMessage(), ex);
      try {
        conn.rollback();
      }
      catch (Exception ex3) {
