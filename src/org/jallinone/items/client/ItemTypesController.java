 ...
 * <p>Description: This class is the grid controller for item types grid frame.</p>
 ...
 * @version 1.0
 */
public class ItemTypesController extends CompanyGridController {
...
* @return an ErrorResponse value object in case of errors, VOListResponse if the operation is successfully completed
  ...
  public Response updateRecords(int[] rowNumbers,ArrayList oldPersistentObjects,ArrayList persistentObjects) throws Exception {
    Response res = ClientUtils.getData("updateItemTypes",new ArrayList[]{oldPersistentObjects,persistentObjects});
    if (!res.isError()) {
  ...
  public Response deleteRecords(ArrayList persistentObjects) throws Exception {
    Response res = ClientUtils.getData("deleteItemType",persistentObjects.get(0));
    if (!res.isError()) {
      frame.getHierarTreePanel().setEnabled(false);
      frame.getHierarTreePanel().clearTree();
  ...
  public void doubleClick(int rowNumber,ValueObject persistentObject) {
    ItemTypeVO vo = (ItemTypeVO)persistentObject;
    frame.getHierarTreePanel().setEnabled(true);
    frame.getHierarTreePanel().setProgressiveHIE02(vo.getProgressiveHie02ITM02());
    frame.getHierarTreePanel().setCompanyCode( vo.getCompanyCodeSys01ITM02() );
    frame.getHierarTreePanel().reloadTree();
