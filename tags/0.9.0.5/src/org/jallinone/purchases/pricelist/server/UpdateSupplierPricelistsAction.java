    ...
    try {
      ...
      for(int i=0;i<oldVOs.size();i++) {
        ...
        res = new CustomizeQueryUtil().updateTable(
            ...
            context,
            true,
            ApplicationConsts.ID_SUPPLIER_PRICELIST_GRID
        );
        if (res.isError()) {
          conn.rollback();
          return res;
      ...
      Logger.error(userSessionPars.getUsername(),this.getClass().getName(),"executeCommand","Error while updating existing supplier pricelists",ex);
      try {
        conn.rollback();
      }
      catch (Exception ex3) {
