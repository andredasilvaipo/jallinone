 ...
 * <p>Description: Help class used to update document totals, according to the specified customer discounts.</p>
 ...
 * @version 1.0
 */
public class UpdateSaleDocTotalDiscountBean {
  ...
  public static final Response updateTotals(SaleDocPK docPK,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
    ...
    try {
      ...
      Logger.error(userSessionPars.getUsername(),"org.jallinone.sales.documents.headerdiscounts.server.UpdateSaleDocTotalDiscountBean","updateTotals","Error while updating document totals:\n"+ex.getMessage(), ex);
      try {
        conn.rollback();
      }
      catch (Exception ex3) {
