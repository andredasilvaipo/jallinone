...
public class ETLProcessThread extends Thread {
  ...
  private void startProcess() {
    ...
    try {
      ...
      Response res = langsBean.loadLanguages(conn,null);
      if (res.isError())
        return;
      java.util.List langsVO = ((VOListResponse)res).getRows();

      // read from SYS24 table...
      res = fieldsBean.loadETLProcessFields(conn,processVO.getProgressiveSYS23(),null);
      if (res.isError())
        return;
      java.util.List fieldsVO = ((VOListResponse)res).getRows();

      Response answer = bean.importData(conn,processVO,fieldsVO,langsVO,null);
      if (answer.isError()) {
        conn.rollback();
        return;
      }
