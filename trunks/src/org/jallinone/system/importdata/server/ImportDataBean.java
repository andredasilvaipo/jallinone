...
import org.jallinone.hierarchies.server.LoadHierarchyBean;
import javax.swing.tree.DefaultTreeModel;
import javax.swing.tree.DefaultMutableTreeNode;
import org.jallinone.hierarchies.java.HierarchyLevelVO;
import org.jallinone.hierarchies.server.InsertLevelBean;
...
public class ImportDataBean {
  ...
  private InsertLevelBean levelsBean = new InsertLevelBean();
    ...
    try {
      ...
      if (processVO.getProgressiveHIE02()!=null) {
        ...
        for(int i=0;i<langsVO.size();i++) {
          ...
          Response res = bean.loadHierarchy(conn,vo.getLanguageCodeSYS09(),processVO.getProgressiveHIE02());
          if (!res.isError()) {
            DefaultTreeModel model = (DefaultTreeModel)((VOResponse)res).getVo();
...
//        }
      ...
      if (processVO.getLocalFile() != null &&
        ...
        if (!f.exists())
          return new ErrorResponse("File non exists");
          ...
          if (rset.next()) {
            ...
            if (processVO.getProgressiveHIE02()!=null) {
              ...
              if (progressiveHIE01==null) {
                ...
                progressiveHIE01 = insertLevels(conn,userSessionPars,langsVO,levelsDesc,processVO.getLevelsSepSYS23(),treeIndexes,processVO.getProgressiveHIE02());
              }
...
//              updPstmt[k].setObject(pos++,params[k].get(i));
            ...
            if (impVO.getProgressiveSys10Fields().size()>0) {
              ...
              while(it.hasNext()) {
                ...
                progressiveSYS10 = CompanyProgressiveUtils.getInternalProgressive(((JAIOUserSessionParameters)userSessionPars).getDefCompanyCodeSys01SYS03(),"SYS10_TRANSLATIONS","PROGRESSIVE",conn);
...
//              insPstmt[k].setObject(pos++,row[i]);
   ...
   */
  private BigDecimal insertLevels(Connection conn,UserSessionParameters userSessionPars,List langsVO,String[] levelsDesc,String levelsSeparator,HashMap[] treeIndexes,BigDecimal progressiveHIE02) throws Exception {
    BigDecimal progressiveHIE01 = null;
    ...
    for(int j=0;j<splittedLevelsDesc[0].length;j++) {
      ...
      if (progressiveHIE01==null) {
        ...
        res = levelsBean.insertLevel(conn,vo,userSessionPars);
        if (!res.isError()) {
